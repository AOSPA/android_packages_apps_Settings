...
class SatelliteAppItemTest {
    ...
    fun setUp() {
        ...
        `when`(applicationInfo.loadLabel(packageManager)).thenReturn(defaultAppLabel)
    }

    @Test
    fun getAppLabel_withCustomLabel_returnsCustomLabel() {
        val customLabel = "Custom Label"
        val item = SatelliteAppItem(applicationInfo, null, customLabel)
        assertThat(item.getAppLabel(packageManager)).isEqualTo(customLabel)
    }
