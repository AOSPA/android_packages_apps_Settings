...
class DarkModeStartTimePreferenceTest {
    ...
    @Before
    fun setUp() {
        mockUiModeManager.stub { on { customNightModeStart } doReturn LocalTime.of(7, 20) }
    ...
    @Test
    fun key() {
        assertThat(preference.key).isEqualTo("dark_theme_start_time")
    }

    @Test
    fun getTitle() {
        assertThat(preference.title).isEqualTo(R.string.night_display_start_time_title)
    }

    @Test
    fun getSummary() {
        assertThat(preference.getSummary(context))
            .isEqualTo(TimeFormatter(context).of(mockUiModeManager.customNightModeStart))
    }
    ...
    fun performClick_showDialog_updateNightModeEndWhenClickDialogSaveBtn() {
        ...
        fragmentScenario.onFragment { fragment ->
            ...
            verify(mockUiModeManager).customNightModeStart = LocalTime.of(7, 20)
    ...
    fun onStop_settingChanges_doNotNotifyPrefChange() {
        ...
        verify(mockLifecycleContext, never()).notifyPreferenceChange(StartTimePreference.KEY)
    }
