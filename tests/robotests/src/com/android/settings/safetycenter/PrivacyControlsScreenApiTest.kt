...
class PrivacyControlsScreenApiTest {
    ...
    @Test
    @EnableFlags(Flags.FLAG_CATALYST_MIGRATION_26Q2, Flags.FLAG_ENABLE_SAFETY_CENTER_NEW_UI)
    fun privacyLocationAccess_get_returnsTrue() {
        assertThat(tester.get<Boolean>("privacy_location_access")).isTrue()
    }

    @Test
    @EnableFlags(Flags.FLAG_CATALYST_MIGRATION_26Q2, Flags.FLAG_ENABLE_SAFETY_CENTER_NEW_UI)
    fun healthConnect_entryPresent_isAvailable() {
        shadowSafetyCenterManager.setSafetyCenterData(
            createScData(
                entries =
                    listOf(
        ...
        assertThat(tester.get<Boolean>("health_connect")).isTrue()
    }

    @Test
    @EnableFlags(Flags.FLAG_CATALYST_MIGRATION_26Q2, Flags.FLAG_ENABLE_SAFETY_CENTER_NEW_UI)
    fun healthConnect_entryMissing_isUnavailable() {
        shadowSafetyCenterManager.setSafetyCenterData(EMPTY_SC_DATA)
        assertFailsWith<FailedPreconditionException> { tester.get<Boolean>("health_connect") }
    }

    @Test
    @EnableFlags(Flags.FLAG_CATALYST_MIGRATION_26Q2, Flags.FLAG_ENABLE_SAFETY_CENTER_NEW_UI)
    fun appFunctionAccess_entryPresent_isAvailable() {
        shadowSafetyCenterManager.setSafetyCenterData(
            createScData(
                entries =
                    listOf(
                        createEntry(
                            id = "app_function_access",
                            title = "app function access",
                            sourceId = "AndroidAppFunctionAccess",
                        )
                    )
            )
        )
        assertThat(tester.get<Boolean>("app_function_access")).isTrue()
    }

    @Test
    @EnableFlags(Flags.FLAG_CATALYST_MIGRATION_26Q2, Flags.FLAG_ENABLE_SAFETY_CENTER_NEW_UI)
    fun appFunctionAccess_entryMissing_isUnavailable() {
        shadowSafetyCenterManager.setSafetyCenterData(EMPTY_SC_DATA)
        assertFailsWith<FailedPreconditionException> { tester.get<Boolean>("app_function_access") }
    }

    @Test
    @EnableFlags(Flags.FLAG_CATALYST_MIGRATION_26Q2, Flags.FLAG_ENABLE_SAFETY_CENTER_NEW_UI)
    fun locationDataSharingUpdates_entryPresent_isAvailable() {
        shadowSafetyCenterManager.setSafetyCenterData(
            createScData(
                entries =
                    listOf(
                        createEntry(
                            id = "location_data_sharing_updates",
                            title = "data sharing updates",
                            sourceId = "AndroidPrivacyAppDataSharingUpdates",
                        )
                    )
            )
        )
        assertThat(tester.get<Boolean>("location_data_sharing_updates")).isTrue()
    }

    @Test
    @EnableFlags(Flags.FLAG_CATALYST_MIGRATION_26Q2, Flags.FLAG_ENABLE_SAFETY_CENTER_NEW_UI)
    fun locationDataSharingUpdates_entryMissing_isUnavailable() {
        shadowSafetyCenterManager.setSafetyCenterData(EMPTY_SC_DATA)
        assertFailsWith<FailedPreconditionException> {
            tester.get<Boolean>("location_data_sharing_updates")
        }
    }
}
