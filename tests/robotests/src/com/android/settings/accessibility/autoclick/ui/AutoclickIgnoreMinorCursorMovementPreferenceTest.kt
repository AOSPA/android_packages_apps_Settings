...
/** Tests for [AutoclickIgnoreMinorCursorMovementPreference]. */
@RunWith(RobolectricTestRunner::class)
class AutoclickIgnoreMinorCursorMovementPreferenceTest {
    @get:Rule val settingsStoreRule = SettingsStoreRule()
    private val context: Context = ApplicationProvider.getApplicationContext()
    private val secureStore by lazy { SettingsSecureStore.get(context) }
    private lateinit var preference: AutoclickIgnoreMinorCursorMovementPreference

    @Before
    fun setUp() {
        preference = AutoclickIgnoreMinorCursorMovementPreference()
    }

    @Test
    fun key_returnsCorrectValue() {
        assertThat(preference.key)
            .isEqualTo(Settings.Secure.ACCESSIBILITY_AUTOCLICK_IGNORE_MINOR_CURSOR_MOVEMENT)
    }

    @Test
    fun purpose_returnsAutoclickIgnoreMinorCursorMovementPurpose() {
        assertThat(context.getString(preference.purpose))
            .isEqualTo(
                context.getString(R.string.a11y_autoclick_ignore_minor_cursor_movement_purpose)
            )
    }

    @Test
    fun title_returnsAutoclickIgnoreMinorCursorMovementTitle() {
        assertThat(context.getString(preference.title))
            .isEqualTo(context.getString(R.string.autoclick_ignore_minor_cursor_movement_title))
    }
    ...
    fun storage_returnsSettingsSecureStoreWithDefault() {
        ...
        assertThat(store.getDefaultValue(preference.key, Boolean::class.javaObjectType))
            .isEqualTo(AccessibilityManager.AUTOCLICK_IGNORE_MINOR_CURSOR_MOVEMENT_DEFAULT)
    }

    @Test
    fun performClick_whenTrue_setsFalse() {
        secureStore.setBoolean(
