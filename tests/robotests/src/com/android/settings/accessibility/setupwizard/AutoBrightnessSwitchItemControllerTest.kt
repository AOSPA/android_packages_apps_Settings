...
class AutoBrightnessSwitchItemControllerTest {
    ...
    private val controller: AutoBrightnessSwitchItemController =
        AutoBrightnessSwitchItemController(context, mockSwitchItem, mockDataStore)
    ...
    fun onStart_registersObserverWithCorrectKey() {
        ...
        verify(mockDataStore)
            .addObserver(eq(AutoBrightnessScreen.KEY), any<KeyedObserver<String>>(), any())
    }
    ...
    fun onStop_removesRegisteredObserver() {
        ...
        verify(mockDataStore).removeObserver(eq(AutoBrightnessScreen.KEY), eq(capturedObserver))
    }
    ...
    fun bindData_setsInitialSwitchStateFromDataStore() {
        mockDataStore.stub { on { getBoolean(eq(AutoBrightnessScreen.KEY)) } doReturn true }
    ...
    fun onItemSelected_togglesSwitchState() {
        activityScenarioRule.scenario.onActivity { activity ->
            ...
            verify(mockDataStore).setBoolean(eq(AutoBrightnessScreen.KEY), eq(true))
        }
    }
    ...
    fun dataStoreChange_triggersRebind() {
        ...
        captor.firstValue.onKeyChanged(AutoBrightnessScreen.KEY, 0)

        verify(mockDataStore, atLeastOnce()).getBoolean(eq(AutoBrightnessScreen.KEY))
    }
