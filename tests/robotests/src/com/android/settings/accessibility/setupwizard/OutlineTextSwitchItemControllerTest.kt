...
/** Tests for [OutlineTextSwitchItemController]. */
@RunWith(RobolectricTestRunner::class)
class OutlineTextSwitchItemControllerTest {
    ...
    private val controller: OutlineTextSwitchItemController by lazy {
        OutlineTextSwitchItemController(context, mockSwitchItem, mockDataStore)
    }
    ...
    fun onStart_registersObserverWithCorrectKey() {
        ...
        verify(mockDataStore)
            .addObserver(eq(OutlineTextPreference.KEY), any<KeyedObserver<String>>(), any())
    }
    ...
    fun onStop_removesRegisteredObserver() {
        ...
        verify(mockDataStore).removeObserver(eq(OutlineTextPreference.KEY), eq(capturedObserver))
    }
    ...
    fun bindData_setsInitialSwitchStateFromDataStore() {
        mockDataStore.stub { on { getBoolean(eq(OutlineTextPreference.KEY)) } doReturn true }
    ...
    fun onItemSelected_togglesSwitchState() {
        activityScenarioRule.scenario.onActivity { activity ->
            ...
            verify(mockDataStore).setBoolean(eq(OutlineTextPreference.KEY), eq(true))
        }
    }
    ...
    fun dataStoreChange_triggersRebind() {
        ...
        captor.firstValue.onKeyChanged(OutlineTextPreference.KEY, 0)

        verify(mockDataStore, atLeastOnce()).getBoolean(eq(OutlineTextPreference.KEY))
    }
