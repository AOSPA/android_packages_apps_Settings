...
class ColorCorrectionPreviewPreferenceTest {
    private val appContext: Context = ApplicationProvider.getApplicationContext()
    private val preference = ColorCorrectionPreviewPreference()

    @Test
    fun createWidget_isPaletteListPreference() {
        val widget = preference.createWidget(appContext)

        assertThat(widget).isInstanceOf(PaletteListPreference::class.java)
    }

    @Test
    fun bindWidget_widgetIsNotSelectable() {
        val widget = preference.createWidget(appContext)
    ...
    fun getKey_returnsCorrectValue() {
        assertThat(preference.key).isEqualTo(ColorCorrectionPreviewPreference.KEY)
    }
}
