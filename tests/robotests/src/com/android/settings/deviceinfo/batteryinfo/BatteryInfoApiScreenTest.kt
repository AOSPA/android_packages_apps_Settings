...
class BatteryInfoApiScreenTest {
    ...
    private val tester = ApiTester(BatteryInfoApiScreen())
    private val context: Context = ApplicationProvider.getApplicationContext()

    private val provider: BatterySettingsFeatureProvider =
        FakeFeatureFactory.setupForTest().batterySettingsFeatureProvider

    @Before
    fun setUp() {
        provider.stub { on { isBatteryInfoEnabled(context) } doReturn true }
    }
