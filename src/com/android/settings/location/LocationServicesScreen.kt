...
open class LocationServicesScreen : PreferenceScreenMixin, PreferenceAvailabilityProvider {
    ...
    override fun getLaunchIntent(context: Context, metadata: PreferenceMetadata?): Intent? =
        makeLaunchIntent(context, ScanningSettingsActivity::class.java, metadata?.key)

    override fun getPreferenceHierarchy(context: Context, coroutineScope: CoroutineScope) =
        preferenceHierarchy(context) {}

    companion object {
        const val KEY = "location_services"
    }
}
// LINT.ThenChange(LocationServices.java, LocationServicesPreferenceController.java)
