...
public class ThemeSettings extends PreviewSeekBarPreferenceFragment implements Indexable {
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mValues = getResources().getIntArray(
                com.android.settingslib.R.array.color_picker_dialog_colors);
        mEntries = getResources().getStringArray(R.array.color_name);

        mDefaultValue = Settings.Secure.getInt(getContentResolver(),
                Settings.Secure.THEME_ACCENT_COLOR, 0);
    }

    /**
     * Persists the current color and writes the value
     */
    @Override
    protected void commit() {
        if (mValues[mCurrentIndex] != mDefaultValue) {
            Settings.Secure.putInt(getContentResolver(),
                    Settings.Secure.THEME_ACCENT_COLOR, mValues[mCurrentIndex]);
        }
    }
