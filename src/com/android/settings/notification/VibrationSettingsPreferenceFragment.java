...
public class VibrationSettingsPreferenceFragment extends DashboardFragment 
    ...
    private void updateIntensityText() {
        ...
        setText(mHapticIntensity, Settings.System.getIntForUser(mContext.getContentResolver(),
            Settings.System.HAPTIC_FEEDBACK_INTENSITY, 2, UserHandle.USER_CURRENT));
    }
    ...
    private void updateRingerPrefs() {
        boolean ringVibEnable = Settings.System.getInt(mContentResolver, Settings.System.VIBRATE_WHEN_RINGING, 0) == 1;
        ringVibEnable |= Settings.Global.getInt(mContentResolver, Settings.Global.APPLY_RAMPING_RINGER, 0) == 1;
        if (ringVibEnable) {
            mRingerVibrationIntensity.setEnabled(true);
            mRingPatternPref.setEnabled(true);
        } else {
            mRingerVibrationIntensity.setEnabled(false);
            mRingPatternPref.setEnabled(false);
        }
    }
        ...
        public void onChange(boolean selfChange, Uri uri) {
            if (!selfChange) {
                if (uri.equals(Settings.System.getUriFor(Settings.System.RING_VIBRATION_INTENSITY))) {
                    ...
                    if (mVibrator != null) {
                        mVibrator.vibrate(200, mAudioAttributesNotif);
                    }
