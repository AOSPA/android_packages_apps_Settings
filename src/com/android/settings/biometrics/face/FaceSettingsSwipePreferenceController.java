...
public class FaceSettingsSwipePreferenceController
    ...
    public boolean isChecked() {
        int defaultValue = mContext.getResources().getBoolean(
                com.android.internal.R.bool.config_faceAuthOnSecurityView) ? 1 : 0;
        return Settings.Secure.getIntForUser(mContext.getContentResolver(),
                Settings.Secure.FACE_UNLOCK_ALWAYS_REQUIRE_SWIPE, defaultValue, getUserId()) != 0;
    }

    @Override
    public boolean setChecked(boolean isChecked) {
        return Settings.Secure.putInt(mContext.getContentResolver(),
                Settings.Secure.FACE_UNLOCK_ALWAYS_REQUIRE_SWIPE, isChecked ? 1 : 0);
    }

    @Override
    public void updateState(Preference preference) {
        super.updateState(preference);
        preference.setEnabled(FaceSettings.isFaceHardwareDetected(mContext)
