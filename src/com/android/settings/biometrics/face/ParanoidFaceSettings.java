    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        if (preference == mKeyguardPref) {
            Settings.Secure.putIntForUser(getContext().getContentResolver(),
                    FACE_UNLOCK_KEYGUARD_ENABLED, Integer.parseInt((String) newValue), UserHandle.USER_CURRENT);
            return true;
        } else if (preference == mAppPref) {
            Settings.Secure.putIntForUser(getContext().getContentResolver(),
                    FACE_UNLOCK_APP_ENABLED, Integer.parseInt((String) newValue), UserHandle.USER_CURRENT);
            return true;
        } else if (preference == mConfirmPref) {
            Settings.Secure.putIntForUser(getContext().getContentResolver(),
                    FACE_UNLOCK_ALWAYS_REQUIRE_CONFIRMATION, Integer.parseInt((String) newValue), UserHandle.USER_CURRENT);
            return true;
        } else if (preference == mBypassPref) {
            Settings.Secure.putInt(getContext().getContentResolver(),
                    FACE_UNLOCK_DISMISSES_KEYGUARD, Integer.parseInt((String) newValue));
            return true;
        }
        updatePrefs();
        return false;
