 ...
 */
@SearchIndexable
public class ParanoidFaceSettings extends SettingsPreferenceFragment implements 
    ...
    private static final String KEY_FOOTER = FooterPreference.KEY_FOOTER;
    ...
    private VideoPreference mVideoPref;
    ...
    private final DialogInterface.OnClickListener mOnClickListener
        ...
        public void onClick(DialogInterface dialog, int which) {
            if (which == DialogInterface.BUTTON_POSITIVE) {
                ...
                if (!mSenseConnector.hasEnrolledFaces()) {
                    Log.e(TAG, "No faces");
                    return;
                }

                mSenseConnector.removeEnrolledFaces();
                if (mSenseConnector.hasEnrolledFaces()) {
                    mRemoveButton.setEnabled(false);
                }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        addPreferencesFromResource(R.xml.security_settings_face_sense);
        ...
        mProvider = FeatureFactory.getFactory(getContext()).getFaceFeatureProvider();

        mVideoPref = findPreference(KEY_VIDEO);
        mVideoPref.onViewVisible(false /* paused */);
    ...
    public void onResume() {
        ...
        if (mVideoPref != null) {
            mVideoPref.onViewVisible(mVideoPaused);
        }

        boolean hasEnrolled = mSenseConnector.hasEnrolledFaces();
    ...
    public void onPause() {
        super.onPause();
        if (mVideoPref != null) {
            mVideoPaused = mVideoPref.isVideoPaused();
            mVideoPref.onViewInvisible();
        }
    }
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        ...
        if (preference == mKeyguardPref) {
            Settings.Secure.putIntForUser(getContext().getContentResolver(),
                    FACE_UNLOCK_KEYGUARD_ENABLED, isEnabled ? 1 : 0, UserHandle.USER_CURRENT);
            return true;
        } else if (preference == mAppPref) {
            Settings.Secure.putIntForUser(getContext().getContentResolver(),
                    FACE_UNLOCK_APP_ENABLED, isEnabled ? 1 : 0, UserHandle.USER_CURRENT);
            return true;
        } else if (preference == mConfirmPref) {
            Settings.Secure.putIntForUser(getContext().getContentResolver(),
                    FACE_UNLOCK_ALWAYS_REQUIRE_CONFIRMATION, isEnabled ? 1 : 0, UserHandle.USER_CURRENT);
            return true;
        } else if (preference == mBypassPref) {
            Settings.Secure.putInt(getContext().getContentResolver(),
                    FACE_UNLOCK_DISMISSES_KEYGUARD, isEnabled ? 1 : 0);
            return true;
        }
        updatePrefs();
        return false;
    ...
    private void updateFooter() {
        final Intent helpIntent = HelpUtils.getHelpIntent(
                getContext(), getContext().getString(R.string.help_url_face), getClass().getName());
        final AnnotationSpan.LinkInfo linkInfo =
                new AnnotationSpan.LinkInfo(getContext(), "url", helpIntent);

        final int footerRes = mProvider.isAttentionSupported(getContext())
                ? R.string.security_settings_face_settings_footer
                : R.string.security_settings_face_settings_footer_attention_not_supported;

        mFooterPref.setTitle(AnnotationSpan.linkify(
                getContext().getText(footerRes), linkInfo));
    }
