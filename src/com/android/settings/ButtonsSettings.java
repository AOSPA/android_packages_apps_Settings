...
public class ButtonsSettings extends SettingsPreferenceFragment implements
    ...
    private void reload() {
        ...
        final boolean swipeUpHomeGestureEnabled = Settings.Secure.getIntForUser(resolver,
                ...
                res.getBoolean(com.android.internal.R.bool.config_swipe_up_gesture_default) ? 1 : 0,
        ...
        if (mAppSwitchLongPressAction != null) {
            if (navigationBarEnabled && swipeUpHomeGestureEnabled) {
                mAppSwitchLongPressAction.setEnabled(false);
            } else {
                mAppSwitchLongPressAction.setEnabled(hasAppSwitch);
            }
        }
        if (mAppSwitchDoubleTapAction != null) {
            if (navigationBarEnabled && swipeUpHomeGestureEnabled) {
                mAppSwitchDoubleTapAction.setEnabled(false);
            } else {
                mAppSwitchDoubleTapAction.setEnabled(hasAppSwitch);
            }
        }
