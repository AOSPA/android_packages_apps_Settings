...
import com.android.settings.paranoid.service.SmsCallHelper;
...
public class QuietHours extends SettingsPreferenceFragment implements
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (getPreferenceManager() != null) {
            ...
            boolean aospDialer =
                Settings.System.getInt(resolver, Settings.System.AOSP_DIALER, 0) == 1;
            ...
            // Remove incoming call sound related options if google dialer is present
            if (!aospDialer) {
                mQuietHoursRing.setEnabled(false);
                mQuietHoursRing.setChecked(false);
                Settings.System.putInt(resolver, Settings.System.QUIET_HOURS_RINGER, 0);

                PreferenceGroup qh_bypass =
                        (PreferenceGroup) findPreference("quiethours_bypass");
                qh_bypass.removePreference((ListPreference) findPreference(KEY_CALL_BYPASS));
                qh_bypass.removePreference((ListPreference) findPreference(KEY_REQUIRED_CALLS));
                qh_bypass.removePreference((RingtonePreference) findPreference(KEY_BYPASS_RINGTONE));
                qh_bypass.removePreference(findPreference(KEY_LOOP_BYPASS_RINGTONE));
            }
