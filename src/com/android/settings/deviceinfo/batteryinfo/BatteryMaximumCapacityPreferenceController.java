...
public class BatteryMaximumCapacityPreferenceController extends BasePreferenceController {
    ...
    public CharSequence getSummary() {
        ...
        if (currentCapacityMicroAh != -1 && designCapacityMicroAh != -1) {
            int currentCapacity = trimCapacityValue(currentCapacityMicroAh);
            int designCapacity = trimCapacityValue(designCapacityMicroAh);
    ...
    private int trimCapacityValue(int capacity) {
        return capacity < 1_000
                ? capacity
                : Integer.parseInt(String.valueOf(capacity).substring(0, 4));
    }
}
